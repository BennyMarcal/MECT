...
def validate_access_token(access_token, provider):
    if provider == 'github':
        ...
        introspection_endpoint = "https://wso2-gw.ua.pt/token" 
        ...
        response = requests.post(introspection_endpoint, json=data)
        return response.status_code == 200 and response.json().get('active') is True
    else:
        return False
